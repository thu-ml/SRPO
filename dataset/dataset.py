...
class D4RL_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = {'s': self.states[use_index],
                ...
                'fake_a':self.fake_actions[use_index] if hasattr(self, "fake_actions") else 0.0,  # self.fake_actions <D, 16, A>
                'fake_a_':self.fake_next_actions[use_index] if hasattr(self, "fake_next_actions") else 0.0,  # self.fake_next_actions <D, 16, A>
            }
        return data
...
class D4RL_dataset(torch.utils.data.Dataset):
    ...
    def _shuffle_data(self):
        ...
        if hasattr(self, "fake_next_actions"):
            self.fake_next_actions = self.fake_next_actions[indices]
    ...
    def sample(self, batch_size):
        ...
        data = {'s': self.states[self.current_idx:self.current_idx+batch_size],
                ...
                'fake_a':self.fake_actions[self.current_idx:self.current_idx+batch_size] if hasattr(self, "fake_actions") else 0.0,  # self.fake_actions <D, 16, A>
                'fake_a_':self.fake_next_actions[self.current_idx:self.current_idx+batch_size] if hasattr(self, "fake_next_actions") else 0.0,  # self.fake_next_actions <D, 16, A>
            }
